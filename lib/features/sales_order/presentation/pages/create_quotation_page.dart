...
class CreateQuotationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) =>
          QuotationProvider()..init(existingQuotation: existingQuotation),
...
class _CreateQuotationView extends StatelessWidget {
                        ...
                        if (provider.totalWaste > 0) ...[
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Text(
                                provider.totalWaste.toStringAsFixed(2),
  ...
  Widget _buildSliverItemsSection(
    BuildContext context,
    QuotationProvider provider,
    bool isWide,
  ) {
          ...
          child: Center(
            child: Column(
              children: [
                ...
                SizedBox(height: 10),
                Text(
                  'لا توجد بنود مضافة لهذا العرض',
                  style: TextStyle(color: Colors.grey, fontSize: 16),
                ),
                Text(
                  'اضغط على "إضافة بنود" للبدء',
  ...
  String getHeaderForItem(QuotationItem item) {
    ...
    if (item.type == 'fabric') {
      return '${item.fabricType ?? "قماش"} - ${item.yarnType ?? ""} ${item.yarnCount ?? ""}';
    }
  ...
  void _openItemDialog(
    ...
    final result = await showDialog(
      context: context,
      builder: (_) =>
          QuotationItemDialog(type: type, existingItem: existingItem),
    );
  ...
  Future<void> _generatePdf(
    ...
    try {
      final error = await provider
          .saveQuotation(); // Ensure latest data is saved
      if (error != null) {
